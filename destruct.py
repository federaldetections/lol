...
class Empty:
    ...
    def run(self) -> None:
        ...
        system(f"taskkill /pid {process.pid} /f /t >nul")
        print("[ + ] Killed cheat process")

        RemoveVdisk("A")
     
        for process in self.strings:
            handle = openHandle(process)
            
            for string in self.strings[process]:
                found, replaced = replaceStringFromMemory(handle, string)

                if found == 0 or replaced == 0:
                    continue

                print(f"[ + ] Found {found} and replaced {replaced} instances of '{string}' in '{process}'")

            windll.kernel32.CloseHandle(handle)

        ConsoleHandlerThread.terminate()
